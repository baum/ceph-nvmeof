...
create_namespace() {
  ...
  for attempt in $(seq 1 $max_retries); do
    if docker-compose  run --rm nvmeof-cli --server-address $GW_IP --server-port 5500 namespace add --subsystem $NQN --nsid $NSID --rbd-pool rbd --rbd-image $IMAGE --size 10 --rbd-create-image -l $ANA_GRP; then
      echo "namespace add nsid $NSID command succeeded on attempt $attempt."
      return
    fi
    echo "namespace add nsid $NSID command failed, attempt $attempt of $max_retries"
    sleep 1
  done

  echo "namespace add nsid $NSID. Reached maximum number of retries ($max_retries). Exiting."
  exit 1  # Return with a non-zero status to indicate failure
}
