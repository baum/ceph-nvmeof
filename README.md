...
1. Download the container images:
    ...
    make pull
    ```
    1. **Alternatively**: for development purposes only, if local changes have been made to the containers, then build them from scratch (it takes 2-3 minutes the first time):
        ```bash
        make build
        ```
...
1. Check that the deployment is up and running:
    ...
    -----------------------------------------------------------------------------------------------------------------------
    ceph-cluster           sh -c ./vstart.sh --new $V ...   Up (healthy)   5000/tcp, 6789/tcp, 6800/tcp, 6801/tcp, 6802/tcp, 6803/tcp, 6804/tcp, 6805/tcp, 80/tcp
    nvmeof_ceph-nvmeof_1   python3 -m control -c ceph ...   Up             0.0.0.0:32797->4420/tcp,:::32797->4420/tcp,0.0.0.0:32796->5500/tcp,:::32796->5500/tcp,0.0.0.0:32795->8009/tcp,:::32795->8009/tcp
    ```
...
DOCKER_BUILDKIT=1 docker-compose exec ceph-cluster bash -c "rbd info demo_image || rbd create demo_image --size 10M"
...
[DPDK require hugepages](https://doc.dpdk.org/guides/linux_gsg/sys_reqs.html#linux-gsg-hugepages) to be set up:
